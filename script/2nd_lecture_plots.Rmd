...
okcupid %>%
        ...
        labs(x = "Income", y = "Height", title = "Scatterplot: Income and Height")
        th
```
# Scatterplot with Regression Line and SE
```{r}
okcupid %>%
        ggplot() +
        aes(income, height) +
        ...
        scale_y_continuous(limits = c(140, 220)) +
        labs(x = "Income", y = "Height", title = "Scatterplot: Income and Height with Regression Line")
        th
```

# Scatterplot with Job Facets
```{r}
okcupid %>%
        filter(job  %in% c("student", "unemployed", "construction / craftsmanship", "political / government")) %>%
        ggplot() +
        aes(income, height) +
        geom_point() +
        scale_y_continuous(limits = c(140, 220)) +
        facet_wrap(~job) +
        labs(x = "Income", y = "Height", title = "Scatterplot: Income and Height Within Each Job")
        th
```
...
okcupid %>%
        ggplot() +
        aes(income, height, colour = sex) +
        geom_point() +
        geom_line() +
        scale_y_continuous(limits = c(140, 220)) +
        labs(x = "Income", y = "Height", title = "Spagetthiplot: Income and Height within Sex") +
        th
```